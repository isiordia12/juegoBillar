...
let canShoot = false; // El jugador NO puede tirar hasta responder bien la pregunta
...
Stick.prototype.handleInput = function (delta) {
    if (!canShoot) return; // Bloquear controles si aún no ha respondido

    if (AI_ON && Game.policy.turn === AI_PLAYER_NUM) return;
    if (Game.policy.turnPlayed) return;

    // Ajustar la potencia del tiro
    ...
    if (this.power > 0 && Mouse.left.down) {
        ...
        // Bloquear disparo hasta la siguiente pregunta
        canShoot = false;

        // Esperar a que la bola se detenga antes de hacer la siguiente pregunta
        let checkBallMovement = setInterval(() => {
            if (!Game.gameWorld.whiteBall.moving) {
                clearInterval(checkBallMovement);
                askQuestion(); // Mostrar nueva pregunta
            }
        }, 500);
